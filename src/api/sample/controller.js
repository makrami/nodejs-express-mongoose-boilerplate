...
async function sampleRegister(req, res) {
  const body = { username: req.username };

  try {
    let user = await User.findOne({ username: body.username });
    if (user) return httpException({ res, message: 'Username is already in use', status: 400 });

    user = await User.create({ username: 'MrAnderson' });
    const payload = { userId: user._id };
    const accessToken = jwt.sign(payload, config.userAccessTokenSecret, {
      expiresIn: config.userAccessTokenSecretExpiresIn,
    });
